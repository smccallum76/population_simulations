...
sql2 = (f"""
       SELECT
        ...
        FROM {table_name}
        WHERE (label IS 'link_left' OR label IS 'link_right') AND (vcf_name NOT IN {vcf_names})
       """)
...
link = link.replace({'label': {'link_left': 'link', 'link_right': 'link'}})  # rename linked data
...
print("The percent of the ihs link_left data that are nans is: ", str(round(nans_link * 100, 2)) + "%")
...
print("Saving Data")
link.to_csv('simulations_4_swifr/3_class/link/link', sep='\t', index=False)
neutral.to_csv('simulations_4_swifr/3_class/neutral/neutral', sep='\t', index=False)
sweep.to_csv('simulations_4_swifr/3_class/sweep/sweep', sep='\t', index=False)
test.to_csv('simulations_4_swifr/3_class/test/test', sep='\t', index=False)
...
train = pd.concat([neutral, link, sweep], ignore_index=True)
train = train.sort_values(by=['vcf_name', 'snp_position'])
# save the train data
train.to_csv('simulations_4_swifr/3_class/train/train', sep='\t', index=False)
